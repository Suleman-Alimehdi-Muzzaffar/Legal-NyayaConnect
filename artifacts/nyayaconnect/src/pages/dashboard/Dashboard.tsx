...
import { mockAppointments, mockDocuments, mockNotifications, mockActivities } from '@/data/dashboardData';
...
const Dashboard = () => {
  ...
  const upcomingAppointments = mockAppointments.filter(a => a.status === 'upcoming').slice(0, 3);
  const pendingDocs = mockDocuments.filter(d => d.status === 'pending_review').slice(0, 3);
  const recentActivities = mockActivities.slice(0, 6);
  const unreadNotifs = mockNotifications.filter(n => !n.isRead).slice(0, 3);
  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants} className="glass-card rounded-3xl p-6 md:p-8 relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-6 border-[#D4AF37]/20 shadow-lg">
        ...
        <div className="relative z-10 flex flex-col items-start gap-2 w-full">
          <h2 className="font-serif text-3xl md:text-4xl font-bold text-white">
            Good Morning, <span className="text-[#D4AF37]">Rahul</span>
          </h2>
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        {[
          { label: "Upcoming Appointments", val: upcomingAppointments.length, sub: "Next: July 18", icon: CalendarCheck },
          { label: "Active Cases", val: "2", sub: "2 in progress", icon: Scale },
          { label: "Pending Documents", val: "2", sub: "Awaiting review", icon: FileText },
          { label: "Total Consultations", val: "12", sub: "Completed", icon: CheckCircle }
        ].map((stat, i) => (
              ...
              {upcomingAppointments.map((apt) => (
                <div key={apt.id} className="glass-card p-4 rounded-2xl border border-white/10 flex flex-col sm:flex-row sm:items-center gap-4 hover:bg-white/[0.07] transition-colors">
                  <div className="flex items-center gap-4 flex-1">
                    <div className={cn("w-12 h-12 rounded-full flex items-center justify-center font-serif text-lg font-bold text-white shadow-inner bg-gradient-to-br", apt.lawyerGradient)}>
                      {apt.lawyerAvatar}
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-3 sm:gap-6 flex-wrap sm:flex-nowrap w-full sm:w-auto">
                    ...
                    {apt.mode === 'online' ? (
                      <button className="bg-[#D4AF37] hover:bg-[#c4a133] text-[#102542] px-4 py-2 rounded-xl text-sm font-bold transition-all hover:-translate-y-0.5 whitespace-nowrap">
                        Join Call
                      </button>
                    ) : (
                      <Link to="/dashboard/appointments" className="bg-transparent border border-white/20 hover:bg-white/10 text-white px-4 py-2 rounded-xl text-sm font-semibold transition-all whitespace-nowrap">
                        View Details
                      </Link>
                    )}
                  </div>
        ...
        <div className="lg:w-80 shrink-0 flex flex-col gap-6 md:gap-8">
          ...
          <motion.div variants={itemVariants} className="flex flex-col gap-4">
            ...
            <div className="glass-card rounded-2xl border border-white/10 flex flex-col divide-y divide-white/10">
              ...
              <button className="py-3 text-center text-xs text-gray-400 hover:text-white transition-colors">
                Mark all as read
              </button>
            </div>
          </motion.div>
