...
const Statistics = () => {
  ...
  const list = lawyers ?? [];
  const totalCases = list.reduce((sum, lawyer) => sum + lawyer.totalCases, 0);
  const cities = new Set(list.map((lawyer) => lawyer.city)).size;
  const avgRating = list.length
    ? +(list.reduce((sum, lawyer) => sum + lawyer.rating, 0) / list.length).toFixed(1)
    : 0;

  const stats = [
    { value: list.length, suffix: '+', label: 'Verified Lawyers' },
    { value: totalCases, suffix: '+', label: 'Cases Handled' },
    { value: cities, suffix: '+', label: 'Cities Covered' },
    { value: avgRating, suffix: '/5', label: 'Average Rating', isFloat: true },
  ];
