...
import { Search, MapPin, SlidersHorizontal, Star, Shield, Building, Video, Languages, ChevronDown, Clock, X, Loader2 } from 'lucide-react';
...
export default function FindLawyers() {
  const { data: lawyersData, isLoading } = useListLawyers();
  ...
  const [aiMatches, setAiMatches] = useState<Array<{ lawyer: any; reason: string }> | null>(null);
  ...
  const [debouncedCity, setDebouncedCity] = useState(cityQuery);
  React.useEffect(() => { const t = setTimeout(() => setDebouncedSearch(searchQuery), 300); return () => clearTimeout(t); }, [searchQuery]);
  React.useEffect(() => { const t = setTimeout(() => setDebouncedCity(cityQuery), 300); return () => clearTimeout(t); }, [cityQuery]);
  React.useEffect(() => { setCurrentPage(1); }, [searchQuery, cityQuery, stateFilter, consultType, selectedAreas, selectedExps, selectedFees, selectedLangs, ratingFilter, sortBy]);
  ...
  const handleAiMatch = async () => {
    if (!aiQuery.trim() || aiQuery.trim().length < 5) return;
    setAiLoading(true);
    setAiMatches(null);
    try {
      const res = await fetch('/api/match', {
        ...
        body: JSON.stringify({ query: aiQuery, city: cityQuery || undefined, language: selectedLangs[0] }),
      });
  ...
  const filteredLawyers = useMemo(() => {
    let result = (lawyersData ?? []).filter(lawyer => {
      ...
      // City / State
      if (debouncedCity && !lawyer.city.toLowerCase().includes(debouncedCity.toLowerCase())) return false;
    ...
    return result;
  }, [lawyersData, debouncedSearch, debouncedCity, stateFilter, consultType, selectedAreas, selectedExps, selectedFees, selectedLangs, ratingFilter, sortBy]);
...
// States
const states = Array.from(new Set([...INDIAN_STATES, ...(lawyersData ?? []).map(l => l.state)])).sort();

  return (
    <div className="min-h-screen bg-[#102542] text-white flex flex-col font-sans">
      {/* Hero / Search */}
      <section className="pt-16 pb-16 bg-gradient-to-b from-[#0a1a2e] to-[#102542] border-b border-white/5">
        <div className="container mx-auto px-6 md:px-12">
          ...
          >
            ...
            <p className="text-gray-400 text-lg mb-8">Search from verified lawyers across India</p>
            ...
            >
              ...
              <button onClick={() => document.getElementById('lawyer-results')?.scrollIntoView({ behavior: 'smooth' })} className="w-full md:w-auto bg-[#D4AF37] hover:bg-[#c4a133] text-[#102542] font-semibold px-8 py-3 rounded-full mt-2 md:mt-0 transition-colors">
                Search
              </button>
            </motion.div>
      ...
      {/* AI Lawyer Match */}
      <section className="container mx-auto px-6 md:px-12 -mt-6 mb-2">
        <div className="glass-card rounded-2xl p-5 border border-[#D4AF37]/20 bg-gradient-to-br from-[#D4AF37]/5 to-transparent">
          <div className="flex flex-col md:flex-row gap-4 items-start md:items-end">
            <div className="flex-1 w-full">
              <label className="text-sm font-semibold text-[#D4AF37] mb-1 block">AI Lawyer Match — describe your case in English/Hindi</label>
              <textarea value={aiQuery} onChange={(e) => setAiQuery(e.target.value)} placeholder="e.g. Meri property par kabza ho gaya, Delhi me 2BHK, budget ₹2000..." rows={2} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-500 outline-none focus:border-[#D4AF37]/50 resize-none text-sm" />
            </div>
            <button onClick={handleAiMatch} disabled={aiLoading || aiQuery.trim().length < 5} className="bg-[#D4AF37] hover:bg-[#c4a133] disabled:opacity-40 text-[#102542] font-bold px-6 py-3 rounded-xl whitespace-nowrap inline-flex items-center gap-2">
              {aiLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Search className="w-4 h-4" />} AI Match
            </button>
          </div>
          {aiMatches && (
            <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
              {aiMatches.length === 0 ? <p className="text-sm text-gray-400">No matches found. Try broader query.</p> : aiMatches.map(({ lawyer, reason }: any) => (
                <Link key={lawyer.id} to={`/lawyers/${lawyer.slug}`} className="bg-white/5 border border-white/10 rounded-xl p-4 hover:border-[#D4AF37]/30 transition-colors">
                  <div className="font-semibold text-white truncate">{lawyer.name}</div>
                  <div className="text-xs text-gray-400 truncate">{lawyer.specializations?.join(', ') || lawyer.primarySpecialization} • {lawyer.city} • ₹{lawyer.consultationFee}</div>
                  <div className="text-xs text-[#D4AF37] mt-1 line-clamp-2">{reason}</div>
                  <div className="text-xs text-gray-500 mt-1">★ {lawyer.rating} • {lawyer.experience} yrs</div>
                </Link>
              ))}
            </div>
          )}
      ...
      <section className="container mx-auto px-6 md:px-12 py-12 flex-grow flex flex-col md:flex-row gap-8 relative">
        ...
        {/* Results Area */}
        <div id="lawyer-results" className="flex-1 flex flex-col">
          ...
          {isLoading ? (
            <div className="flex-1 flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-gray-400">Finding the right lawyers...</p>
            </div>
