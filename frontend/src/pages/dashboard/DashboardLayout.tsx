...
const DashboardLayout = () => {
  ...
  const initials = (user?.name ?? 'RM').split(' ').map(w => w[0]).join('').slice(0, 2).toUpperCase();
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full bg-[#0a1929] border-r border-white/10 w-64 text-white overflow-y-auto scrollbar-hide">
      ...
      <div className="p-6 border-b border-white/10 flex flex-col items-center text-center shrink-0">
        <div className="w-16 h-16 rounded-full bg-gradient-to-br from-[#D4AF37] to-[#8c7324] flex items-center justify-center text-2xl font-serif font-bold text-[#102542] mb-3 border-2 border-[#102542] ring-2 ring-[#D4AF37]/50 shadow-[0_0_15px_rgba(212,175,55,0.3)] overflow-hidden">
          {user?.avatar ? <img src={avatarUrl(user.avatar)} alt={user.name} className="w-full h-full object-cover" /> : initials}
        </div>
        <h3 className="font-serif text-lg font-bold">{user?.name ?? 'Client'}</h3>
        <p className="text-sm text-gray-400 truncate w-full">{user?.email ?? 'Not signed in'}</p>
      ...
      <div className="p-4 border-t border-white/10 shrink-0 flex flex-col gap-2">
        <button
          ...
          className="flex items-center gap-3 px-4 py-2 font-sans text-sm font-medium text-red-400 hover:text-red-300 hover:bg-red-400/10 transition-colors w-full rounded-lg"
        >
      ...
      <main className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden relative">
        ...
        <header className="h-16 shrink-0 bg-[#102542]/80 backdrop-blur-md border-b border-white/10 sticky top-0 z-30 flex items-center justify-between px-4 md:px-6">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <Link to="/dashboard/profile" className="w-10 h-10 rounded-full bg-gradient-to-br from-[#D4AF37] to-[#8c7324] flex items-center justify-center font-serif text-sm font-bold text-[#102542] border-2 border-[#102542] ring-2 ring-[#D4AF37]/50 md:ml-2 overflow-hidden">
              {user?.avatar ? <img src={avatarUrl(user.avatar)} alt={user.name} className="w-full h-full object-cover" /> : initials}
            </Link>
          </div>
        </header>
