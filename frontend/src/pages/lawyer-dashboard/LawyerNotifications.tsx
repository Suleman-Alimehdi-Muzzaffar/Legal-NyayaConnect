...
import { useListLawyerNotifications } from '@workspace/api-client-react';
import type { LawyerNotification } from '@workspace/api-client-react';
import { Link } from 'react-router-dom';

const LawyerNotifications = () => {
  const [activeTab, setActiveTab] = useState('all');
  const { data: notificationsData } = useListLawyerNotifications();
  const [notifications, setNotifications] = useState<LawyerNotification[]>(notificationsData ?? []);
  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case 'payment': return <IndianRupee className="w-5 h-5 text-green-400" />;
      case 'review': return <Star className="w-5 h-5 text-yellow-400" />;
      case 'hearing': return <Gavel className="w-5 h-5 text-orange-400" />;
  ...
  const getIconBg = (type: string) => {
    switch (type) {
      ...
      case 'hearing': return 'bg-orange-500/10 border-orange-500/20';
  ...
  const getBorderColor = (type: string) => {
    switch (type) {
      ...
      case 'hearing': return 'border-l-orange-500';
