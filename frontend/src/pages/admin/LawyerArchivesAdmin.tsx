...
const LawyerArchivesAdmin = () => {
  ...
  const lawyerExports = ((dataExports ?? []) as unknown as Array<DataExport & { kind?: string }>).filter(
    (d) => d.kind === 'lawyer_zip',
  );
        ...
        <main className="flex-1 flex items-center justify-center p-6">
          <div className="w-full max-w-md bg-[#0a1929] border border-[#D4AF37]/30 rounded-2xl p-8 shadow-[0_0_30px_rgba(212,175,55,0.1)]">
            ...
            <p className="text-sm text-gray-400 mb-6">
              Enter the admin key to review lawyer ZIP archive requests. Set <code className="text-[#D4AF37]">ADMIN_KEY</code> in backend/.env on the server.
            </p>
            <input
              type="password"
              value={keyInput}
    ...
    <div className="min-h-screen bg-[#102542] text-white font-sans flex flex-col">
      <header className="h-16 bg-[#0a1929] border-b border-white/10 flex items-center justify-between px-6 shrink-0">
        ...
        <div className="flex items-center gap-4">
          <nav className="flex items-center gap-1 text-sm">
            ...
            <Link to="/admin/lawyer-archives" className="px-3 py-1.5 rounded-lg bg-[#D4AF37]/10 text-[#D4AF37] font-semibold transition-colors">
              Lawyer ZIP Archive
            </Link>
          </nav>
          <button
      ...
      <main className="flex-1 p-6 max-w-6xl w-full mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="font-serif text-3xl font-bold mb-1">Lawyer ZIP Archive Requests</h1>
            <p className="text-gray-400 text-sm">
              Grant or deny requests from lawyers who want a ZIP archive of their data. Only granted lawyers can download.
            </p>
          </div>
          <button
            onClick={() => refetch()}
        ...
        {!isLoading && !error && lawyerExports.length === 0 && (
          <div className="text-center py-24 text-gray-400 border border-dashed border-white/15 rounded-2xl">
            ...
            No lawyer ZIP archive requests yet.
          </div>
        )}

        {!isLoading && !error && lawyerExports.map((d) => (
          <div key={d.id} className="bg-[#0a1929] border border-white/10 rounded-2xl p-6 mb-6">
            <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
              <div>
                <div className="flex items-center gap-3">
                  ...
                  <span className="px-2 py-0.5 rounded text-[10px] font-bold border bg-[#D4AF37]/10 text-[#D4AF37] border-[#D4AF37]/30 uppercase tracking-wider">Lawyer ZIP</span>
                </div>
            ...
            {d.status === 'denied' && (d as unknown as { reason?: string }).reason && (
              <p className="text-sm text-red-300/80 bg-red-500/10 border border-red-500/20 rounded-xl p-3 mb-4">
                Reason: {(d as unknown as { reason?: string }).reason}
              </p>
            )}
            ...
            {d.status === 'pending' && <div className="mt-4 flex items-center gap-2 text-xs text-gray-500"><Clock className="w-3.5 h-3.5" /> The lawyer sees "pending" until you decide.</div>}
          </div>
        ))}
      </main>
    </div>
...
export default LawyerArchivesAdmin;
